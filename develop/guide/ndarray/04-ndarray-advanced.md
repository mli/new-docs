...
* We can create an NDArray from a regular {.Python .Input} list or tuple by using the `array` function:

```{.python .input}
import mxnet as mx
# create a 1-dimensional array with a {.python .input} list
a = mx.nd.array([1,2,3])
# create a 2-dimensional array with a nested {.python .input} list
...
When assigning an NDArray to another {.Python .Input} variable, we copy a reference to the
...
is to use `pickle`, as you might with any other {.Python .Input} objects. `NDArray` is
pickle-compatible.
...
1. When using these methods, you can save data from within the {.Python .Input} interface
   and then use it later from another language's binding. For example, if we save
   the data in {.Python .Input}:
...
in {.Python .Input}, the {.Python .Input} thread just pushes this operation into the backend engine
and then returns.  There are two benefits to this approach:

1. The main {.Python .Input} thread can continue to execute other computations once the
